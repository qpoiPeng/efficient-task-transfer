...
def run_seq_finetuning(args):
    ...
    sequence = permutations(task_map["all"])
    ...
    for seq in sequence:
        ...
        for task_from, task_to in zip(seq[0:], seq[1:]):
            ...
            if args["overwrite_mode"] == 0 and os.path.exists(output_dir):
                print(f"Skipping task {task_from} as it already exists.")
                continue
            # also skip if appending & max runs reached
            elif (
                args["overwrite_mode"] == 1
                and args["max_restarts"] is not None
                and task_from in results
                and len(results[task_from]["seeds"]) >= args["max_restarts"]
            ):
